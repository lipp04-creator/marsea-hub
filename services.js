...
// mapping location -> rest area id (tanpa mengubah service apa pun)
  ...
  box.innerHTML = `
    ...
    <div class="summary-box">
      ...
      <div style="margin-top:6px;font-size:12px;color:#6b7a8b;">
        Angka di atas bersifat estimasi untuk prototipe MARSEA dan dapat berubah sesuai negosiasi operasional.
      </div>
    </div>

    <div class="modal-footer">
      <button class="btn-back" type="button" onclick="renderNormalStep1()">Kembali</button>
      <button class="btn-next" type="button" onclick="submitBooking()">Konfirmasi Pemesanan</button>
    </div>
  `;
}
...
async function submitBooking() {
  try {
    ...
    const vesselName = `Kapal ${vesselType} (AUTO)`; // nanti bisa diganti input manual
    const qty = STATE.qty || 1;

    const serviceSummary = `${s.name} — ${qty} unit — mode: ${STATE.mode}`;
    ...
    alert("Pemesanan berhasil disimpan. Operator MARSEA akan menindaklanjuti.");
    closeModal();
  } catch (err) {
    console.error(err);
...
function renderEmergencyStep() {
  ...
  box.innerHTML = `
    ...
    <div class="summary-box">
      ...
      <div style="margin-top:6px;font-size:12px;color:#6b7a8b;">
        Biaya akhir akan ditentukan oleh operator MARSEA berdasarkan jenis kejadian dan durasi penanganan.
      </div>
    </div>

    <div class="modal-footer">
      <button class="btn-back" type="button" onclick="closeModal()">Tutup</button>
      <button class="btn-next" type="button" onclick="closeModal()">Hubungi Operator</button>
    </div>
  `;
}
