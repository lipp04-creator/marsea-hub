...
// Dipakai oleh index.html dan map.html, dan menyediakan konstanta JSON global
// untuk halaman lain (mis. dashboard).
...
// Kalau Home ingin hit backend (utilization, bookings, dst)
const HOME_API_BASE = "http://localhost:5000";

document.addEventListener("DOMContentLoaded", () => {
  setupSidebarToggle();
  // Hanya index.html yang punya kpiGrid & ndwiMiniChart, jadi aman dipanggil di semua halaman
  loadHomeOverview();
  loadNdwiMiniChart();
});
...
async function loadHomeOverview() {
  ...
  try {
    const [vesselRes, restRes, ndwiRes, homeRes] = await Promise.all([
      fetch(VESSELS_JSON),
      fetch(`${HOME_API_BASE}/api/restareas/utilization`),
      fetch(NDWI_JSON),
      fetch("home.json")
    ]);
    ...
    // Hitung dari sumber utama (vessels + utilization + ndwi)
    const activeVessels = vessels.length || (meta && meta.vessels_active) || 0;

    let totalCap = 0;
    let totalUsed = 0;
    rests.forEach(r => {
      const cap  = toNumberSafe(r.capacity ?? r.cap);
      const used = toNumberSafe(r.used);
      totalCap  += cap;
      totalUsed += used;
    });

    const restAreasActive = rests.length || (meta && meta.rest_areas_total) || 0;
    const dockedShips     = totalUsed || (meta && meta.docked_ships) || 0;
    const slotsAvailable  = Math.max(totalCap - totalUsed, 0) || (meta && meta.rest_slots_available) || 0;
    ...
    const ndwiDisplay = ndwiLast != null
      ...
      : (meta && meta.ndwi_avg != null ? Number(meta.ndwi_avg).toFixed(2) : "0.75");
